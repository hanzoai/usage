...
import { formatBucket, formatCents, formatCount, formatDeltaPct, deltaDirection } from './format'

// ── palette (categorical, dark+light legible — from the console SERIES set) ──────

const SERIES = ['#6ea8fe', '#7ee787', '#f0a868', '#c792ea', '#56d4c4', '#e879a6', '#d6c15a', '#8b9bb4'] as const
const TRACK = 'rgba(128,128,128,0.18)'
const UP = '#7ee787'
const DOWN = '#e5534b'
const colorAt = (i: number): string => SERIES[i % SERIES.length] as string

// ── dependency-free SVG marks (harvested from @hanzo/ui Metric/Charts) ───────────

/** A single-series sparkline over real points; nothing for <2 points. */
function Sparkline({ points, width = 120, height = 34, color = SERIES[0] }: { points: number[]; width?: number; height?: number; color?: string }): ReactElement | null {
...
function BarSeries({ bars, height = 120, color = SERIES[0] }: { bars: { label: string; value: number; hint: string }[]; height?: number; color?: string }): ReactElement | null {
...
function MeterBar({ pct, color }: { pct: number; color: string }): ReactElement {
...
function Panel({ title, action, children, minW }: { title: string; action?: ReactNode; children: ReactNode; minW?: number }) {
  return (
...
function MetricTile({ icon, label, value, deltaPct, spark, sparkColor }: { icon: ReactNode; label: string; value: string; deltaPct?: number | null; spark?: number[]; sparkColor?: string }) {
